...
public final class SloppyMath {
   ...
   * in this module.
   *
   * @param args Either none, and the log sum rountines are tested, or the
...
//    }
  ...
  public static <T> double[] logScoresToProbs(double[] logX) {
    double logSum = logAdd(logX);
    assert logSum > Double.NEGATIVE_INFINITY;
    double[] ret = DoubleArrays.clone(logX);
    DoubleArrays.addInPlace(ret, -logSum);
    DoubleArrays.exponentiateInPlace(ret);
    return ret;
  }
